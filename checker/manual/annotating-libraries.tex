...
%% TODO: reinstate this once the checker/lib/ directory contains content.
...
% \code{java -jar \$CHECKERFRAMEWORK/checker/dist/checker-framework.jar} both use the appropriate \<jdk7.jar> or \<jdk8.jar>
...
\subsection{The \<-AuseSafeDefaultsForUnannotatedSourceCode> command-line
argument\label{AuseSafeDefaultsForUnannotatedSourceCode}}

\begin{sloppypar}
When compiling a library that is not fully annotated, use command-line
argument \<-AuseSafeDefaultsForUnannotatedSourceCode>.  This causes
the checker to behave normally for classes with a relevant \<@AnnotatedFor>
annotation.  For all other classes, the checker uses conservative defaults
...
using the \<-AuseSafeDefaultsForUnannotatedSourceCode> command-line
...
%     typing those 17 characters later.
  ...
  \item
    ...
    \begin{itemize}
    \item
      passes the \<-AuseSafeDefaultsForUnannotatedSourceCode>
...
\item
  Create a stub file by running the stub class generator.  (\<checker-framework.jar> and \<javac.jar>
...
%   \code{checker-framework.jar} file to contain the skeleton class generator.
%   \code{checker-framework.jar} should already be on your classpath (see
