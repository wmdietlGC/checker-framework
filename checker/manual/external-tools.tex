...
If you ordinarily use the \code{javac} compiler from the command line, then you can
...
\end{Verbatim}
   ...
   run checker-framework.jar via \<java> (not \<javac>) as in:
...
\begin{Verbatim}
  # Unix
  export CHECKERFRAMEWORK=${HOME}/checker-framework-1.9.9
  alias javacheck='java -jar $CHECKERFRAMEWORK/checker/dist/checker-framework.jar'

  # Windows
  set CHECKERFRAMEWORK = C:\Program Files\checker-framework-1.9.9\
  doskey javacheck=java -jar %CHECKERFRAMEWORK%\checker\dist\checker-framework.jar $*
\end{Verbatim}
   ...
   \<\$CHECKERFRAMEWORK/checker/dist/checker-framework.jar>;
...
launched via \<checker-framework.jar> such as when \code{\$CHECKERFRAMEWORK/checker/bin/javac}
is run.  These instructions bypass \<checker-framework.jar> and cause the compiler to run a
